...
def show_single_talk_chart():
    ...
    # Drop non scripture columns from selected row
    scripture_counts = selected_row.drop(labels=["Speaker_Name", "Talk_Name", "Kicker"])

    # Only keep books with at least one reference
...
def menu():
    ...
    if choice == '1':
        ...
        if choice == "1":
            show_all_talks_chart()
        elif choice == '2':
            show_single_talk_chart()
        else:
